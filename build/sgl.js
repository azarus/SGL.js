...
var BufferObject = globalClass(_class = function () {
	function BufferObject(data, size, buffertype, type, drawmode) {
		_classCallCheck(this, BufferObject);
	...
	_createClass(BufferObject, [{
		...
		key: "data",
		value: function data(_data, size) {
			gl.bindBuffer(this.buffertype, this.buffer);
			gl.bufferData(this.buffertype, new Float32Array(_data), this.drawmode);
			this.numItems = _data.length / size;
			this.itemSize = size;
		}
	}, {
		key: "bind",
...
},{"./AnimatedSprite":1,"./Asset":2,"./BlendMode":3,"./BufferObject":4,"./Color":5,"./Drawable":6,"./IndexBuffer":8,"./Input":9,"./Lightmap/Lightmap":11,"./Math":13,"./Matrix":14,"./RenderCanvas":15,"./RenderTarget":16,"./RenderTexture":17,"./Shader":18,"./Sprite":19,"./SpriteSheet":20,"./Texture":21,"./TextureAtlas":22,"./TileMap":23,"./Transform":24,"./Vector":25,"./VertexBuffer":26,"./View":27}],8:[function(require,module,exports){
"use strict";
...
var IndexBuffer = globalClass(_class = function (_BufferObject) {
	_inherits(IndexBuffer, _BufferObject);

	function IndexBuffer(data, size, type, drawmode) {
		_classCallCheck(this, IndexBuffer);

		return _possibleConstructorReturn(this, (IndexBuffer.__proto__ || Object.getPrototypeOf(IndexBuffer)).call(this, data, size, gl.ELEMENT_ARRAY_BUFFER, type, drawmode));
	}
...
var Light = globalClass(_class = function (_Drawable) {
	...
	function Light(lightTexture) {
		var color = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : Color.White;
		var radius = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 128;
		var intensity = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 1;
...
var Lightmap = globalClass(_class = function (_Drawable) {
	...
	function Lightmap(width, height) {
		...
		_this.lights = [];
		_this.shadowHulls = [];
		_this.blockColor = Color.Black;
		_this.shadowColor = Color.Black;
		_this.shadowHullBuffer = null;
		_this.NormalBuffer = null;
		_this.shadowBlockBuffer = null;

		_this.shader = new Shader();

		_this.shader.create("precision mediump float;" + "uniform vec3 uColor;" + "void main(void){ gl_FragColor = vec4(uColor, 1);}", Shader.Pixel);

		_this.shader.create("attribute vec2 aVertexPosition;" + "attribute vec2 aVertexNormal;" + "uniform mat3 uMatrix;" + "uniform vec2 uLightPosition;\n" + "#define SHADOW_RANGE 1500.0\n" + "void main(void) {" +
		// Calculate Shadow Casting Direction
		"vec2 Direction = normalize(aVertexPosition.xy - uLightPosition.xy);" + "vec2 Position = aVertexPosition;" +
		// Translate shadow casting vertices
		"if(dot(aVertexNormal, Direction) > 0.0) {" + "Position = Position + (Direction * SHADOW_RANGE);" + "}" +
	...
	_createClass(Lightmap, [{
		key: "addShadowHull",
		value: function addShadowHull(hull) {
			this.shadowHulls.push(hull);
		}
	}, {
		key: "removeShadowHull",
		value: function removeShadowHull(hull) {
			this.shadowHulls.splice(this.shadowHulls.indexOf(hull), 1);
		}
	}, {
		key: "rebuild",
		value: function rebuild() {
			if (this.shadowHullBuffer) this.shadowHullBuffer.destroy();
			...
			for (var h = 0; h < this.shadowHulls.length; ++h) {
				...
				for (var v = 0; v < vertices.length; v++) {
					...
					// Triangle 1
					flipnormal.x, flipnormal.y, flipnormal.x, flipnormal.y, normal.x, normal.y,

					// Triangle 2
					flipnormal.x, flipnormal.y, normal.x, normal.y, normal.x, normal.y]);

					// Add Vertex Normals
					Vertices.push.apply(Vertices, [currentVertex.x, currentVertex.y, nextVertex.x, nextVertex.y, currentVertex.x, currentVertex.y, nextVertex.x, nextVertex.y, currentVertex.x, currentVertex.y, nextVertex.x, nextVertex.y]);
				}
			}
...
},{"./Light":10,"./ShadowHull":12}],12:[function(require,module,exports){
"use strict";
...
var ShadowHull = globalClass(_class = function () {
	function ShadowHull(vertices) {
		_classCallCheck(this, ShadowHull);
	...
	_createClass(ShadowHull, [{
		key: "getVertices",
		value: function getVertices() {
			return this.vertices;
		}
	}, {
		key: "getVerticesAsVector",
		value: function getVerticesAsVector() {
			var vertices = [];
		...
		key: "makeQuad",
		value: function makeQuad(x, y, w, h) {
			return new ShadowHull([x, y, x + w, y, x + w, y + h, x, y, x + w, y + h, x, y + h]);
		}
	}, {
		key: "makeCircle",
		value: function makeCircle(x, y) {
			var radius = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 32;
			var resolution = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 12;
			...
			for (var i = 0; i < points.length; ++i) {
				...
				vertices.push.apply(vertices, [x + currentVertex.x, y + currentVertex.y, x + nextVertex.x, y + nextVertex.y, x, y]);
			}
...
var RenderTarget = globalClass(_class = function () {
	...
	_createClass(RenderTarget, [{
		...
		key: "drawElements",
		value: function drawElements(buffer) {
			this.bind();
			this.applyViewport();
...
var Vector = globalClass(_class = function () {
	...
	_createClass(Vector, null, [{
		...
		key: "lengthSq",
		value: function lengthSq(vector) {
...
var VertexBuffer = globalClass(_class = function (_BufferObject) {
	_inherits(VertexBuffer, _BufferObject);

	function VertexBuffer(data, size, type, drawmode) {
		_classCallCheck(this, VertexBuffer);

		return _possibleConstructorReturn(this, (VertexBuffer.__proto__ || Object.getPrototypeOf(VertexBuffer)).call(this, data, size, gl.ARRAY_BUFFER, type, drawmode));
	}
...
},{"f:\\Coding\\github/src/Utils/Timer":32}],32:[function(require,module,exports){
"use strict";
...
},{"f:\\Coding\\github/src/Graphics/Graphics":7,"f:\\Coding\\github/src/Utils/Console":28,"f:\\Coding\\github/src/Utils/Decorators":29,"f:\\Coding\\github/src/Utils/EventListener":30,"f:\\Coding\\github/src/Utils/GameLoop":31}]},{},[33]);
